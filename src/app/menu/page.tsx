            ...
            <div className={`${isLoading ? "hidden" : ''}`}>
                ...
                <div className={`min-h-[calc(100vh-100px)] w-full flex justify-center bg-[#e4c3a2] px-2 `}>
                    <div className="h-full max-w-7xl w-full flex gap-4 mt-4">
                        <div className={`flex flex-col items-center  ${userRole === 'admin' || userRole === 'waiter' ? 'gap-2' : 'gap-0'}`}>
                            <Link className={`text-black`} href="/menu/addDish" >
                                <div className={`flex flex-row text-nowrap w-fit bg-white p-2 rounded-2xl ${userRole === 'admin' || userRole === 'waiter' ? '' : 'hidden'}`}>
                                    ДОБАВИТЬ +
                                </div>
                            </Link>
